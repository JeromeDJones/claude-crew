...
> **Resolution (2026-05-20):** three of the four findings below were fixed in `fix/memory-scope-typing` (commit `a0f1f7b`): the `Scope` Literal alias + re-typing, the `build_memory_section` else→raise, and the `cwd=None` integration test. The inline-import finding remains open (deferred — likely a circular-import guard). See `doc/BACKLOG.md` → `[2026-05-19] Feature: multi-scope-agent-memory` for status detail.
