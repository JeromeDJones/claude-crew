...
3. **Project-scoped agent memory across runs** (claude-crew's own feature, `multi-scope-agent-memory` slice on master): persistent teammates accumulate a per-project memory file at `~/.claude/projects/<projectpath>/memory/MEMORY.md`. The teammate writes this file *itself*, mid-session, using the `Write` tool — following the memory guidance injected into its system prompt at spawn (`build_memory_section` + `ensure_write_tool`); claude-crew does not parse the conversation. On a **graceful** termination (`graceful-termination-memory-flush` slice) — an explicit `kill_teammate` (the default) or a `shutdown_all` of a *healthy* teammate that has a memory surface — the broker first grants it one final bounded turn (≤90s) to persist anything outstanding *before* tombstoning it. This final flush is **skipped** for memory-less teammates, for `graceful=False` hard kills, and on unexpected **death** (the subprocess is already gone, so there is no session to flush). Future teammates spawned for the same role + same project inherit the file.
