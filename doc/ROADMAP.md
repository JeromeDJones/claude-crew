...
| **M3.5** — Reshape crew while running | ✅ done (2026-07-01) | `claude_crew/server.py` (+`reshape_crew`), `claude_crew/sdk_teammate.py` (D0 unconditional `send_to`), `claude_crew/broker.py` (+3 helpers), `tests/test_live_reshape.py` (NEW, live) + 4 stub suites | Five verbs applied to a LIVE crew: `set_gate`→`set_edge_override`; `add_node`/`augment` spawn+inform **without respawn** (D4); `drop` kills + sweeps stale overrides (D6); `swap` spawns-records-**then**-kills (no dead-slot window, D5). M1.5 gate reused verbatim. D0: `send_to` wired unconditionally (`authorize_send` is the boundary, not tool presence). 20 ATs incl. 2 live guards; stub 1708/0. |
...
## Next: M4 — RepoReactor as shape

With M3.5 shipped, the substrate (M0/M1.5/M2/M3/M3.5) now supports declarative, human-gated, live-executing AND live-reshapeable crew graphs. The remaining arc items are **M4** (re-author the RR workflow as a declared shape using M2 routing + M3 adaptation) and **M5** (memory-informed autonomous adaptation).

## Shipped: M3.5 — Reshape Live Crew

Apply the five verbs to an already-instantiated topology. `reshape_crew` MCP tool (`server.py`): five-stage flow (base-resolution requiring an INSTANTIATED proposal → verb-guard → role-resolution → `verb.apply` → M1.5 gate) then per-verb live dispatch. `set_gate`→`set_edge_override`; `add_node`/`augment` spawn+`record_topology`+inform running source (no respawn, D4); `drop`→minus-topology + graceful kill + stale-`_edge_overrides` sweep (D6) + inform survivors; `swap`→spawn+record **then** kill (no dead-slot window, D5). D0 unconditional `send_to` wiring (respawn-free edge addition; `authorize_send` is the security boundary). 3 additive broker helpers. 20 ATs incl. 2 live regression guards (first live tests to drive a real teammate `send_to`); stub 1708/0. Shipped 2026-07-01. Spec: `.rr/specs/m3-5-reshape-live-crew.md` (archived at merge).
