...
- **📄 Idea brief (full learnings + recipe)**: [`doc/ideas/local-model-backed-teammates.md`](ideas/local-model-backed-teammates.md) — self-contained, plannable.
- **What**: We validated that a local GPU model (Gemma-4 via llama.cpp + claude-code-router) can drive Claude Code agentic sessions (tool use + multi-turn, ~7s/turn). Full recipe + caching gotchas in `doc/ideas/local-model-backed-teammates.md`. Today the integration is **all-or-nothing per crew process**: SDK teammates inherit the parent's `os.environ`, so launching the crew with `ANTHROPIC_BASE_URL` → ccr routes *every* teammate to the local model. There's no way to run a **mixed** crew — e.g. lead + reviewers on Opus, bulk/mechanical teammates on the local model.
- **Where**: `claude_crew/sdk_teammate.py` builds `ClaudeAgentOptions(**opts_kwargs)` (~line 1221) with **no `env`/`base_url`** set — it relies purely on inherited process env. To support per-teammate routing, thread an optional `env` (or explicit `base_url`/`api_key`) override from spawn-time through `broker.py` (`spawn_teammate` factory signature already carries `model`/`effort`/`cwd`) and `factories.py` into the SDK options. Confirm `claude-agent-sdk`'s `ClaudeAgentOptions` exposes an `env` passthrough to the CLI subprocess (verify against installed SDK; if not, may need a subprocess-mode path).
- **Why it matters**: Cost + privacy + throughput. Run cheap/parallelizable work (mechanical refactors, test runs, bulk edits) on free local compute while keeping high-stakes reasoning on Anthropic. Also enables on-prem/air-gapped crews. This is the natural payoff of the local-model validation.
- **Concurrency constraint (design in)**: one local GPU = effectively **one concurrent heavy session** (`run-gemma.sh` uses `--parallel 1`; concurrent teammates serialize and evict each other's KV prefix cache). A mixed crew should cap local-routed teammates to the slot count, or stand up multiple llama.cpp servers + ccr providers. See concurrency caveat in `doc/ideas/local-model-backed-teammates.md`.
- **Prereq gotcha**: local routing requires `CLAUDE_CODE_ATTRIBUTION_HEADER=0` (rotating telemetry hash at char 0 of the system prompt otherwise busts prefix caching → ~3-4 min/turn). Whatever sets per-teammate env must also set this for local-routed teammates.
- **Suggested size**: **M** (spawn-time env override plumbed through 3 layers + SDK env-passthrough verification + tests at broker and sdk_teammate layers; the cost driver is confirming the SDK honors per-spawn env and not regressing the inherited-env default).
