...
| 27 | **Fidelity-audit live-test suite.** A single live-gated test class (`CLAUDE_CREW_LIVE_TESTS=1`) per CLI-fidelity claim, asserting that teammates obey Claude Code's rules: bundled-pack dispatch, skill discovery, hook firing (with explicit env-var carve-out), plugin scope filter, MCP resolution from user-level config, agent format YAML polymorphism, frontmatter normalization. Run pre-release. Subsumes [2026-05-08] bundled-pack dispatch one-off. Makes the CLI-fidelity differentiator into an asserted commitment, not incident response. | 1, 4 | 1 | M | **idea** | Routed 2026-05-12 from BACKLOG reorg + vision-pass review. The bundled-pack dispatch bug (2026-05-07→05-08) cleared 978 unit tests with zero signal — that's the moat eroding. See BACKLOG entry [2026-05-12] for suite-contents detail + implementer notes (API-cost gating, SDK-version pinning, prior-art at `test_format_compat_e2e.py::TestLiveSdkToolsEmptyEnforcement`, Windows-fix xfail handling, shell-env-var carve-out as explicit invariant). |
