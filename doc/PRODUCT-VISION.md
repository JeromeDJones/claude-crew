...
| 26 | **Plugin agent config visibility on the dashboard.** Operator report 2026-05-07: when claude-crew runs in a project with a project-scope plugin install (`scope: "local"` with `projectPath`), spawning a teammate from that plugin succeeds but the dashboard config panel renders empty — no chips, no expanded detail. The teammate works; only the transparency surface is broken. Suspected: divergence between the spawn factory's synthetic-AgentDef path and `_resolve_agent_def`'s None-return path (factory.py:294-301 vs :362-365), or project-root mismatch silently dropping the plugin pack at startup. Phase 1 spike planned to identify root cause before Phase 2. Spec at `doc/features/FEATURE-plugin-agent-config-ui-visibility.md`. | 4 | 4 | S | **idea** | Filed 2026-05-07. Three live hypotheses; spike planned. Acceptance for Phase 1: failing test reproducing the symptom + inline root-cause writeup before Phase 2. |
