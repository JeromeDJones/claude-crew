...
| 3c | **Cross-session teammate memory (full mirror).** Teammates read and write `~/.claude/projects/<encoded>/memory/MEMORY.md` and per-entry `*.md` files using the same structure Claude Code's auto-memory subsystem uses (index file + frontmatter-tagged entries with `name`/`description`/`type`). Loaded into the teammate on spawn via system prompt or initialPrompt; appended-to during work via explicit instructions in the pack's prompts. Same path Claude Code uses, so a teammate's memory stays compatible if the SDK ever activates auto-memory natively. Per-teammate-identity scoping (each role gets its own memory file) is the default; an opt-in shared mode is a follow-up. | 1 | 1 | M | gated on #5 retro | Empirical findings in `doc/research/sdk-memory.md`: SDK doesn't auto-activate the subsystem, but the path is fully writable from teammates. **Gate (set 2026-04-26):** evaluate post-#5 retro based on what real work actually surfaced — did manual rescues correlate with cross-session memory absence, or with something else (observability gaps, multi-crew bugs, etc.)? If memory wasn't the load-bearing gap, defer further. Cross-session continuity is what makes "feels like a collaborator" real, but the design surface (scoping, eviction, four-type discipline) is M-effort and shouldn't precede empirical signal from #5. |
