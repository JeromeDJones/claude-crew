...
**Features Implemented**: 8 (MVP + #6 telemetry-based liveness + #7 subagent-activity envelopes + #8 tool-execution telemetry); #9 next
...
| 11 | **Lightweight subagent context.** Explorer, planner, and general-purpose bundled agents load full user CLAUDE.md via `setting_sources=["user","project"]` by default — wasted tokens for utility roles. Add `setting_sources` to `PackFrontmatter` so roles can declare `setting_sources: []`. Update bundled explorer and general-purpose to use minimal context. Requires a parallel channel alongside the agents pack (AgentDefinition has no `setting_sources` field). | 1, 2 | 1 | S | idea | Surfaced during #10 design session (2026-04-28). Ships after #10 — both touch `_loader.py`. See `doc/features/FEATURE-lightweight-subagent-context.md`. |
