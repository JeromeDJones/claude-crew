...
Makes crew shapes first-class, declarative, and human-gated. M0 shipped 2026-06-11; M1.5 shipped 2026-06-12; M2 shipped 2026-06-13. Agreed execution order: **M0 done → M1.5 done → M2 done → M1 next** (Jerome). Rationale: M1.5 fixed the gate's *interaction model* (non-blocking, dual-channel, resurfaceable); M2 makes the approved graph *execute* (routing enforcement, scoped send, neighbor injection, circuit breaker, dashboard overlay); M1 (shape templates, classifier) is independent and higher-value after the graph runs.
...
| wsc-m1 | **Workflow Shape Composition M1.** Blessed shape library + lead router/classifier: `micro-fix`, `standard-feature`, `heavy-feature` template shapes in a `shapes/` dir; `propose_shape` accepts a file path; lead classifies a problem and selects a shape. Independent of M2 — no routing logic involved. Higher-value after the graph runs. | 1 | 1 | M | **next** | M2 shipped 2026-06-13. Execution order revised 2026-06-11 (after M0); confirmed after M2 completion. |
