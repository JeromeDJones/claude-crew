...
**Next up**: workflow-shape-composition-**m1.5** (async/non-blocking shape-gate: dual-channel UI-or-chat approval, notify-lead-on-resolve, resurfaceable artifact-style surface — brief: `doc/design/M1.5-async-shape-gate-BRIEF.md`), then m2 (edge routing). Agreed ordering (Jerome): M0 done → **M1.5 next** → M2 → M1 later.
...
Makes crew shapes first-class, declarative, and human-gated. M0 shipped 2026-06-11. Agreed execution order: **M0 done → M1.5 next → M2 → M1 later** (Jerome; M1.5 inserted 2026-06-12 after live verification). Rationale: M1.5 fixes the gate's *interaction model* (non-blocking, dual-channel, resurfaceable) — daily-use friction hit live; it's self-contained (no edge-routing dependency). M2 then makes the approved graph *execute*; M1 (shape templates, classifier) is independent and higher-value after the graph runs.
...
| wsc-m1.5 | **Async shape-gate (NEXT).** Reframe the M0 gate's *interaction model* (visual unchanged): `propose_shape` becomes **non-blocking** (surfaces + returns `shape_id`, lead stays free for chat); **dual-channel approval** — dashboard Approve/Decline **or** a new `resolve_shape(shape_id, decision)` lead tool so the operator approves *over chat/phone*; **notify-lead-on-resolve** via the broker inbox channel (no polling/blocking); **resurfaceable** artifact-style surface (tray/badge/notification, survives navigation) reusing the `surface_document` machinery. Preserves the gate invariant (no spawn without human approval). | 1, 4 | 1 | M | **next** | Brief: `doc/design/M1.5-async-shape-gate-BRIEF.md` (+ design `shape-gate-async-surface.md`). From live verification 2026-06-12 (Jerome). Self-contained; supersedes the deferred active-instance §6 finding. Reuses the shipped cross-instance modal as the visual. NOTE: M0 tests assume blocking `propose_shape` — update for non-blocking. |
| wsc-m2 | **Workflow Shape Composition M2 (after M1.5).** Edge routing enforcement: `tee`/`direct`/`gated` modes actually constrain message flow. Scoped teammate `send_to` — messages only traverse declared edges. Neighbor adjacency injection — each spawned teammate's context includes its declared neighbors. Circuit breaker. Makes the approved graph execute; M0's `Topology` becomes the live routing rail. Edge animation/promotion in the dashboard also M2. | 1, 4 | 1 | L | **after M1.5** | Agreed after M0/M1.5 (Jerome). Plugs directly into `BrokerSnapshot.topologies` + `Topology.edges` rail M0 laid. |
