...
| 15 | **Native Claude Code agent format compatibility.** Operators distributing claude-crew bring rich `~/.claude/agents/*.md` definitions; substrate's job is to consume them, not bundle alternatives. Shipped: `name:` silent-accept + canonical role key (file stem fallback); `model:` optional (SDK default when absent — wire-safe per `client.py:157` None-filter); `tools:` optional with safe-by-default empty tuple (NOT SDK-inherit, which would silently grant full tool access); `tools:` and `disallowedTools:` accept comma-string-or-list YAML polymorphism (closes a latent character-iteration bug at `_loader.py:315` that shipped pre-#15); `color:` silent-accept; system prompt composition revised — `SUBSTRATE_SUBAGENT_GUIDANCE` LEADS, agent body APPENDED via new `build_subagent_prompt(body)` helper. Teammate path retains body-first/addendum-last ordering (deliberate carve-out — peer-list injection requires body to land first). Closes noisy-startup-WARN regression from #17 BACKLOG M-2: zero unsupported-key WARNs against real `~/.claude/agents/runner.md` (verified live). Bundled packs (`general_purpose.md`, `explorer.md`, `planner.md`) declare `name:` matching their existing role keys — substrate dogfoods its own contract. Deferred to #24 / hook-aware feature: `hooks:` per-agent lifecycle hooks (Claude Code DOES support them; vision corrected); `isolation: worktree`. | 1, 2 | 4 | S-M | **done** | Shipped 2026-05-02 on `feature/claude-code-agent-format-compat` — 6 commits (Phase 1-3 spec + 5 implementation tasks T1-T5). 754 tests passing, zero regressions, live dogfood verified. Spec at `doc/features/FEATURE-claude-code-agent-format-compatibility.md`. |
| 16 | **Message kind typing in transcript stream.** Transcript envelopes currently all emit `kind: "msg"`. Inspect payload shape to emit `kind: "tool"` for tool-call envelopes and `kind: "thinking"` for thinking-block envelopes. Dashboard stream columns then show the visual distinction (monospace pill vs. italic) the design intends. Requires coordination between broker envelope format and `UIServer._build_local_instance()`. | 4 | 3 | S | **done (2026-05-06, thinking half cut)** | `kind: "tool"` shipped via #19 (`ui_server.py:308`). `kind: "thinking"` cut deliberately — `ThinkingBlock` responses are silently dropped by `sdk_teammate.py` (A2, line 159), and extended-thinking mode is rare in standard usage; no real-task run has surfaced a need to see thinking content in the dashboard. Revisit only if a future operator workflow actually demands it; the dispatch path stays simple in the meantime. |
| 17 | **Agent definition parity (mcpServers + memory in PackFrontmatter; subagent + teammate paths both wired).** `PackFrontmatter` was missing two AgentDefinition fields silently — `mcpServers` and `memory` (the original vision row claimed four; reconnaissance proved `permissionMode` and `disallowedTools` were already wired since #10). Additive field extensions to `PackFrontmatter` + shallow validation rejecting `sdk`-type inline dicts (require in-process callable, can't survive YAML). On the spawn path: `_resolve_mcp_servers` translates pack list-form → `ClaudeAgentOptions.mcp_servers` dict (string-name resolution against `~/.claude.json`, inline-dict pass-through with `name` strip, INFO breadcrumb on every pass-through). `memory` is subagent-context-only with WARN at teammate spawn (no `ClaudeAgentOptions` carrier). Also: `spawn_teammate` MCP tool now rejects invalid `permission_mode` via `ToolError` at the protocol boundary; `_warn_unknown_mcp_servers` mirrors `_warn_unknown_skills` for load-time validation; `_warn_shadow_drop` closes the pre-existing whole-replacement footgun across all 9 optional AgentDefinition fields. | 1, 2 | 2 | S | **done** | **Shipped 2026-05-01.** Full SDD: 11 SCs, 13 design decisions D-1 through D-13, 5 tasks Kael direct + sentinel checkpoint at T1-T3 boundary + final. 51 new tests across 5 files, 700 passing. Co-architect three-pushback warmup at Phase 1 raised four pillars (subagent-vs-teammate bifurcation, mcpServers translation semantics, validation depth, multi-source shadow precedence) all encoded as D-decisions before Phase 2 synthesis. See `doc/features/FEATURE-agent-definition-parity.md`. |
| 18 | **Broker `snapshot()` read API + dashboard polish.** Add `broker.snapshot()` returning a frozen dataclass (`crew_id`, `alive_teammates`, `log`) so `UIServer` stops reading `_info`, `_log`, `_teammates` directly. Removes fragile private-attr coupling and makes UIServer unit-testable without a live broker. Bundle with two small dashboard fixes: git branch read at UIServer init (`git branch --show-current`, cached, fallback "main") instead of hardcoded `"main"`; and remove the stale `_get_redaction_version()` ImportError fallback in `teammate.py`. | 4 | 2 | S | **done** | **Shipped 2026-04-30** (`2a93977`). `BrokerSnapshot` frozen dataclass with deep-copied status; `_build_local_instance(snapshot)` decoupled from broker internals; `tool_events` reserved field for #19. Surfaced #21 during build (co-architect token burn diagnosis). See journal entry. |
| 19 | **Tool-use events in the dashboard stream.** Surface `tool_start`/`tool_end` JSONL transcript records in the Mission Control message columns alongside envelope messages. Requires a broker-level per-teammate event list (or a parallel channel alongside `_log`) that `UIServer` reads and merges into the transcript stream, emitting `kind: "tool"` entries the dashboard already knows how to render. SDK teammates already write these records to the JSONL sink — the gap is getting them into the in-memory state the UI polls. **Now consumes the BrokerSnapshot reserved field `tool_events: tuple = ()` added in #18 (D-10/SC-13), so no snapshot v2 needed.** | 4 | 4 | M | **done** | **Shipped 2026-05-01** (`db2d0ab`). Full SDD: 11 SCs, 13 design decisions D-1 through D-13, 5 tasks Kael direct + sentinel checkpoint at T2-T3 boundary + final. 35 new tests, 585 passing. ToolEvent dataclass mirrors #8 JSONL shape; bounded per-teammate deque; `_tombstone_teammate` step 8c captures via `dataclasses.replace`; UIServer merges inline by raw float timestamp; Task tool filtered at render time (preserved upstream). No `/api/state` schema change — events ride inside `transcripts[crew_id]`. See `doc/features/FEATURE-tool-events-dashboard-stream.md`. |
| 21 | **Teammate prompt parity (subagent vs. teammate context).** When a role like `general-purpose` is spawned as a top-level **teammate** (not a Task subagent), `SdkTeammate` uses a generic 8-word default system prompt (`f"You are a {role}. Help the lead with {role}-level work."`) and discards the entire pack-file body. Result: teammates get no role identity, no contract, no awareness of available subagents — even though they DO have Task tool access (the SDK adds it implicitly when `agents=` is passed). The pack body's "you have no Task tool by design — subagents are leaves" assertion is also a lie when applied to teammate context. Fix: factor the leaf assertion into a `_LEAF_SUFFIX` constant the subagent loader appends; pack body becomes shared role identity (true in both contexts); teammates get pack body + a `_TEAMMATE_ADDENDUM` containing context override, dynamic peer list (from `agents` dict), delegation framework, and anti-patterns. | 1, 2 | 4 | S-M | **done** | **Shipped 2026-04-30** (`1e3cfdc`). 4 commits on feature branch: T1 prompt assembly + leaf factor, T2 15-test parity suite, T3 cite-vs-research delegation clause + live-test pre-checks, plus F14 token-telemetry bug fix discovered during live delegation testing. Live verified: clean general-purpose teammate dispatched explorer subagent for research-shaped probe (`last_subagent_completed` populated). 550 tests passing. See `doc/features/FEATURE-teammate-prompt-parity.md`. |
| 22 | **Agent-header `current_tool` badge prominence boost.** Today the in-flight tool name is plaintext in a secondary scrolling status row below the agent name (`AgentStreamColumn` lines 467-508), with no animation on the tool name itself (only the status dot pulses). #19 (tool-events stream) intentionally rendered completed events only — in-flight visibility is this badge's job. The "operator stares at silence during a 90s Bash" gap from #19's Phase 1 problem statement is partially open until this badge work lands: promote `current_tool` to the pinned agent header alongside the avatar/name; add a subtle pulse or shimmer when a tool has been in-flight >5s; surface elapsed seconds so an operator sees "Bash · 23s" growing. | 4 | 5 | S | **done** | **Shipped 2026-05-01.** Full SDD, Kael direct: 11 SCs, 11 design decisions D-1 through D-11, 5 tasks (T1 server payload, T2 tombstone-race test, T3 dashboard rendering, T4 D-8 freeze comment, T5 E2E pipeline) + sentinel checkpoint at T3-T4 boundary + final pass. Two-layer signal: column-top accent bar (D-1, 3px, `--st-tool` tinted, pulse ≥5s) for cross-column scan; dedicated 22px chip row (D-2, always reserved — SC-8 layout-stable across rapid Pre/Post pairs) showing `oldest_in_flight · {N}s [+M]` for in-column investigation. Clock pairing via per-instance `now_wallclock` and per-agent `oldest_in_flight.started_at_wallclock` from same `time.time()` (D-4); client tick uses `performance.now()` + `setInterval(1000)` to be NTP/suspend-immune (D-5). 25 new tests, 610 passing, 0 regressions. Producer side untouched. See `doc/features/FEATURE-current-tool-badge-prominence.md`. |
| 23 | **Global skills support for SDK teammates.** User-defined skills in `~/.claude/skills/*/SKILL.md` and project `.claude/skills/*/SKILL.md` are invocable by SDK teammates the same way the lead Claude Code session invokes them. | 1, 2 | 4 | S | **done** | **Shipped 2026-05-01** (`4c959fe`). Phase 1 spike collapsed scope from M→S — claude-agent-sdk 0.1.68 natively exposes skills via `ClaudeAgentOptions.skills` and `AgentDefinition.skills`; #10's `PackFrontmatter.skills` plumbing was already wired through `sdk_teammate.py:893`. A-1 (disallowedTools cascade into skill bodies) resolved empirically by probe — no escalation. Bundled `general-purpose` ships `skills: all` + `settingSources: ["user","project"]`; explorer/planner stay narrow (SC-10). 10 SCs, 9 design decisions D-1..D-9, 5 tasks Kael direct + sentinel mid-build + final MERGE-WITH-NOTE. 21 new tests, 649 passing. See `doc/features/FEATURE-global-skills-for-sdk-teammates.md`. |
| 25 | **Startup diagnostics surfaced on the dashboard.** Today, server-startup-time WARNs (pack shadowing, unknown-skill names from #23 SC-4, unsupported frontmatter keys from `strict_parse`) go to stderr only — the Mission Control dashboard cannot show them because pack-load happens BEFORE any teammate envelope exists. Operators currently must tail the claude-crew server stderr to know if their config is broken. Add a startup-diagnostics channel: collect WARNs/INFOs emitted during `build_merged_pack` and similar startup paths into a structured list on `BrokerSnapshot` (or a sibling reserved field), surface in `/api/state`, render in the dashboard as a dismissable "startup notices" panel. Persists across the session so operators see the issue even if they connect after startup. Surfaces the same channel for skill-not-found WARNs, agent-frontmatter typos, MCP-config issues, and any future startup-time diagnostic. | 4 | 4 | M | **done (2026-05-06)** | Shipped via RepoReactor on 2026-05-06. Full SDD: 11 ATs, 4-task chain (`diagnostics-domain` → `broker-snapshot-field` → `factory-capture-wire` → `ui-payload-and-panel`), all slice-reviewed PASS, feature-review PASS, validation PASS. 44 new tests (25 domain, 9 broker, 5 ui_server unit, 4 Playwright E2E) + 963 existing tests green. Reserved-field pattern from #18 mirrored as `BrokerSnapshot.startup_diagnostics: tuple[StartupDiagnostic, ...] = ()`; capture via `logging.Handler` in `default_factory()` co-located with `build_merged_pack()`; OQ-1 propagation probe + direct-attach fallback for silenced loggers; six-category classifier (shadow/unknown_skill/unknown_mcp_server/frontmatter/plugin/other); WARN+ default with INFO toggle; panel hidden when empty; visual idiom mirrors `TerminatedSection`. Two slice.scope.task-touches-violation deviations both adjudicated as breakout planning gaps. Five Low/deferred follow-ups in BACKLOG. See `doc/features/FEATURE-startup-diagnostics-dashboard.md`. |
| 24 | **Subagent-dispatch routing guard + post-output validation hook.** Programmatic defense against the fail-soft fabrication pathology surfaced 2026-05-01 (BACKLOG entries [2026-05-01] x2). Two-part: (1) `PreSubagentUse` hook on the parent inspects `subagent_type` against the dispatched task text — if shell-verbs (`find`, `wc`, `grep`, `exec`, `\|`) appear in the prompt and the target role's tool surface lacks Bash, refuse the dispatch with an error envelope BEFORE the model reaches the subagent. (2) `PostSubagentUse` / output-validation hook detects fabrication shapes (markdown path/line tables, shell-output blocks) in subagent results when no matching tool fired in the turn — fail the envelope, do not pass it up. Folds in F7 telemetry gap fix (Agent dispatch in redaction allowlist; subagent_type captured in args_summary) so misroutes are visible. Layer 1 (peer-list tools, shipped `bd052ef`) is the data signal this enforces. | 1, 2, 4 | 5 | M | **rejected (2026-05-06)** | Rejected after design walkthrough. Both halves are pattern-based defenses against a probabilistic adversary: Part 1 needs heuristic intent-detection (the regex set leaks); Part 2 has structural ground truth via the F8 tool ledger but only catches shape-bearing fabrications (path/line tables, shell-output blocks) — the prose-fabrication class ("the file has 47 functions") is out of reach for any pattern-based check. Net result: ongoing detector-curation burden for a partial defense, with false-positive risk that erodes operator trust faster than fail-softs do. Better-leverage alternatives: tighten pack contracts on the specific roles that actually fail; treat subagent prose as untrusted-by-default at the operator/review layer; close the F7 `Agent`-vs-`Task` telemetry gap as a standalone small fix so dispatches are at least observable. The BACKLOG entries that motivated this stay open and route to those alternatives instead. |
| 26 | **Plugin agent config visibility on the dashboard.** Operator report 2026-05-07: when claude-crew runs in a project with a project-scope plugin install (`scope: "local"` with `projectPath`), spawning a teammate from that plugin succeeded but the dashboard config panel rendered empty — no chips, no expanded detail. Root cause (confirmed via spike): `_read_installed_plugins` did strict `projectPath` equality, silently dropping plugin agents whenever claude-crew ran in a *subdirectory* of the plugin's `projectPath` instead of at-or-below it. Fix: prefix containment via `is_relative_to`; new `plugin_scope_miss` startup-diagnostic category surfaces the rejection on the dashboard's Startup Notices panel. Spec at `doc/features/FEATURE-plugin-agent-config-ui-visibility.md`. | 4 | 4 | S | **done (2026-05-07)** | **Shipped 2026-05-07** via RepoReactor. Phase 1 spike pinpointed root cause (subdirectory-not-at-root in plugin scope filter). Phase 2 single-task slice: prefix-containment filter + `plugin_scope_miss` diagnostic category with classifier rule ordered ahead of the existing `installPath` rule. 8 ATs + 978 existing tests green; all RR phase verdicts PASS (plan/breakout/slice/feature/validation). H1 synthetic-AgentDef divergence (separate code-read finding) split off to BACKLOG. |
| 20 | **Peer messaging between teammates.** Teammates gain a `send_to(teammate_id, text)` MCP tool so peer A can message peer B without the lead routing every turn. Architecture: broker exposes a Unix-socket MCP endpoint; each `SdkTeammate` is spawned with `ClaudeAgentOptions.mcpServers` pointing at that socket. Sender id auto-stamped from the teammate's identity (no spoofing). Recipient dead/tombstoned returns the existing `teammate_dead` error from #6. Lead receives a copy of every peer envelope (preserves observability — lead stays orchestrator, not bottleneck). Hop counter (TTL=4) on envelope drops runaway A→B→A→B loops with a logged warning. v1 is `send_to` only; `broadcast` and teammate-side `get_messages` deferred. | 1, 2 | 5 | M | **backlogged (2026-05-17)** | Surfaced 2026-04-30 during F14 session; backlogged 2026-05-17 after a 5-lens design brainstorm (wire, MVP, UX, security, skeptic). Decision rationale: **the coordinator-in-the-loop is the moat, not the bottleneck.** Routing every cross-teammate exchange through the coordinator (the lead session driving the crew) is what enables judgement calls about what gets fixed and when "good enough" is reached — RepoReactor's implementor↔slice-reviewer loop is the canonical example, and coordinator-mediated routing is the feature, not the friction. The motivating workflow ("builder→reviewer handoff without lead intermediation") has no logged friction in real sessions; coordinator-mediated routing is performing well today. Peer messaging would route *around* the only consistent gating layer that catches fail-soft fabrications (BACKLOG 2026-05-01 entries) and erodes the substrate's strategic position ("human-supervised crew with deep observability") toward "autonomous crew that talks to itself" — a crowded space where claude-crew has no edge. May revisit if/when logged friction emerges. The brainstorm also surfaced that the vision row's load-bearing socket-transport assumption is incorrect: `SdkTeammate` already holds `self._broker` in-process, so any future revisit should start from in-process closure (unspoofable sender by construction) and not pre-build for out-of-process teammates that don't exist. Adjacent rows ("Recursive crew spawning", "MCP server injection + cwd on spawn_teammate") remain on their own merits. |
| 27 | **Fidelity-audit live-test suite.** A single live-gated test class (`CLAUDE_CREW_LIVE_TESTS=1`) per CLI-fidelity claim, asserting that teammates obey Claude Code's rules: bundled-pack dispatch, skill discovery, hook firing (with explicit env-var carve-out), plugin scope filter, MCP resolution from user-level config, agent format YAML polymorphism, frontmatter normalization. Run pre-release. Subsumes [2026-05-08] bundled-pack dispatch one-off. Makes the CLI-fidelity differentiator into an asserted commitment, not incident response. | 1, 4 | 1 | M | **done (2026-05-16)** | Shipped 2026-05-16 via RepoReactor. Two test modules: `tests/test_fidelity_audit.py` (1,091 LOC, 9 classes, live-gated) + `tests/test_fidelity_audit_frontmatter.py` (154 LOC, 1 class, default-CI). 7 live fidelity claims + auth-failure surface + per-test cost artifact at `tests/_artifacts/fidelity-audit-cost.jsonl`. Validation PASS: `10 passed, 1 xfailed in 72.13s`, ~$0.35. `_preserve_sdk_auth` helper added (surgical fix) — HOME-monkeypatch strips SDK credentials; copies `~/.claude/.credentials.json` + `~/.claude.json` into tmp HOME before spawning. Both deferred gaps closed 2026-05-16 by `fidelity-audit-followups` slice: `discover_dir` extended to glob `*.yaml`/`*.yml` with `parse_yaml_pack_text` end-to-end through `build_merged_pack`; `_record_sdk_cost` helper now populates real `ResultMessage.usage` data across all 7 non-auth live classes (7/9 non-zero cost lines per validation run). Re-validated 10p+1xfail in 72s, ~$0.35. See `doc/features/FEATURE-fidelity-audit-suite.md` for the updated gaps table. |
...
Makes crew shapes first-class, declarative, human-gated, and now adaptable. M0 shipped 2026-06-11; M1.5 shipped 2026-06-12; M2 shipped 2026-06-13; M3 shipped 2026-06-17. Agreed execution order: **M0 done → M1.5 done → M2 done → M3 done → M3.5 next** (Jerome). M1 (blessed shape library + classifier) is **reframed as repo-react policy** — the substrate (M0/M2/M3) provides the mechanism; repo-react picks and right-sizes shapes. Rationale: M1.5 fixed the gate's *interaction model* (non-blocking, dual-channel, resurfaceable); M2 makes the approved graph *execute* (routing enforcement, scoped send, neighbor injection, circuit breaker, dashboard overlay); M3 makes adaptation *computable* (five pure verb commands, iteratively human-gated, pre-instantiation only); M3.5 reshapes a live/running crew.
...
| wsc-m1.5 | **Async shape-gate.** Reframe the M0 gate's *interaction model* (visual unchanged): `propose_shape` becomes **non-blocking** (surfaces + returns `shape_id`, lead stays free for chat); **dual-channel approval** — dashboard Approve/Decline **or** a new `resolve_shape(shape_id, decision)` lead tool so the operator approves *over chat/phone*; **notify-lead-on-resolve** via the broker inbox channel (no polling/blocking); **resurfaceable** artifact-style surface (tray/badge/notification, survives navigation). Preserves the gate invariant (no spawn without human approval). | 1, 4 | 1 | M | **done (2026-06-12)** | 4 tasks, 6 ATs; 1461 full-suite green (exit 0). 14→16 tools (+`resolve_shape`, +`list_pending_shapes`). Single choke-point notify in `broker.resolve_proposal`. Resurfaceable `MCTopBar` badge/tray gate. Human-in-the-loop guarantee mechanical→trust-enforced (bridge to M1). `instantiate_shape` refusal of non-approved unchanged. Live UX verified by Jerome 2026-06-12. |
| wsc-m2 | **Workflow Shape Composition M2.** Edge routing enforcement: `tee`/`direct`/`gated` modes actually constrain message flow. Scoped teammate `send_to` — messages only traverse declared edges (in-process FastMCP tool on `SdkTeammate`; sole teammate→broker choke-point). Neighbor adjacency injection — each spawned teammate's system prompt includes its declared neighbors (derived from same `shape.edges` as authorization — cannot drift). Budget-only circuit breaker (`CIRCUIT_BREAKER_MAX_EXCHANGES=8`; no deadlock detector — not well-defined at message-bus level). On-graph overlay + promote control in Mission Control dashboard (multi-instance proxied). | 1, 4 | 1 | L | **done (2026-06-13)** | 4 tasks, 13 ATs; +75 feature tests (edge-routing 19, circuit-breaker 17, scoped-send 22, edge-dashboard 17); 1537 full-suite green (exit 0). `broker.py` (routing engine + circuit breaker + `send_scoped` + `EdgeStat`), `sdk_teammate.py` (in-process `send_to` MCP), `factories.py` (`neighbors=` threading), `teammate_prompt.py` (neighbor section), `ui_server.py` (+3 routes), `dashboard.html` (`TopologyEdgePanel` SVG-walk overlay). |
| wsc-m1 | **Workflow Shape Composition M1.** Blessed shape library + lead router/classifier: `micro-fix`, `standard-feature`, `heavy-feature` template shapes in a `shapes/` dir; `propose_shape` accepts a file path; lead classifies a problem and selects a shape. Independent of M2 — no routing logic involved. | 1 | 1 | M | **↪ reframed → repo-react** | Reframed 2026-06-17: blessed shapes + right-sizing/classification is **policy**, now owned by repo-react. The substrate (M0/M2/M3) provides the mechanism; repo-react picks and right-sizes shapes. |
| wsc-m3 | **Adaptation algebra.** Five pure verb commands (`AddNode`/`Swap`/`Augment`/`SetGate`/`Drop`) in `shapes.py` — each `apply(shape) → (Shape, AdaptationDiff)`. `adapt_shape` MCP tool reuses the M1.5 gate verbatim (`broker.py` untouched). Pre-instantiation only; roles resolved via same `factory.known_roles` seam as `instantiate_shape`. | 1 | 1 | L | **done (2026-06-17)** | 2 tasks, 35 ATs; 83 feature tests + 1613 full-suite green (exit 0). `shapes.py` (+algebra), `server.py` (+1 tool), `tests/test_shape_adaptation.py` (45, new), `tests/test_shape_adapt_tool.py` (38, new). |
| wsc-m3.5 | **Reshape crew while running.** Apply adaptation verbs to an already-instantiated topology: `swap`/`drop` kill/respawn running teammates; rewire live routing mid-flight. Touches broker live registry + M2 routing engine — materially bigger than M3's pure-data algebra. Carved out of M3 scope 2026-06-17. | 1 | 1 | L | **next** | |
| wsc-m4 | **RepoReactor as declared shape (deferred).** Re-author the RR workflow as a declared shape using M2 routing + M3 adaptation. The convergence point where repo-react's blessed shapes meet claude-crew's shape substrate. | 1 | 1 | L | **deferred** | |
...
M2 makes the approved graph *execute*. The `Topology` recorded by M0's `instantiate_shape` is now the live **routing rail**: messages between teammates are authorized, filtered, and observable according to their declared edge mode.

**Four delivered pieces:**

**1. Edge routing enforcement** (`broker.py`): `_send_routed()` resolves the declared mode for every teammate→teammate message. `direct` delivers only to the recipient's inbox; `tee` delivers to the recipient and sends a CC copy to the lead; `gated` (and the no-declared-edge fallback) wraps the message and routes to the lead only. `promote_edge` allows runtime mode override. `UnauthorizedEdgeError` is raised if a teammate sends to a non-neighbor.

**2. Budget-only circuit breaker** (`broker.py`): `CIRCUIT_BREAKER_MAX_EXCHANGES = 8` per directed edge. Exceeding the budget auto-promotes the edge to `gated`, emits a `{type:"circuit_breaker", edge, reason:"budget_exceeded"}` control envelope to the lead, and reroutes the triggering message as a gated wrapper. Idempotent. **No deadlock detector** — the spec-proposed 2-node pending-flag check was dropped by coordinator adjudication (Jerome + Kael, 2026-06-13): "A waits B waits A" is not well-defined at a message-bus level; reply-clearing makes both-pending unreachable; the budget is the sole runaway guard. `EdgeStat` frozen dataclass surfaces per-edge state (mode, exchanges, tripped) on `BrokerSnapshot.topology_edge_stats`.

**3. Scoped `send_to` in-process MCP tool** (`sdk_teammate.py`): each spawned teammate receives a private in-process FastMCP server (`_build_send_to_mcp_server()`) exposing a single `send_to(recipient, message)` tool that calls `broker.send_scoped`. This is the **sole channel** by which a teammate can message the broker — the moat choke-point. `factories.py` threads a `neighbors=` kwarg so each teammate receives its declared neighbor list at spawn; `teammate_prompt.py` injects a "Neighbors" system-prompt section. The neighbor list is derived from the same `shape.edges` as the authorization check — it cannot drift.

**4. Edge observability in Mission Control** (`ui_server.py`, `dashboard.html`): `/api/state` carries `topology_edge_stats` per crew. `GET /edge-log/{crew_id}/{from}/{to}` returns the routed message log for an edge; `POST /edge-promote/{crew_id}/{from}/{to}` promotes an edge to gated. Both routes are fully multi-instance proxied (leader→follower, same pattern as `/shape-approval`). `TopologyEdgePanel` in `dashboard.html` SVG-walks the mermaid-rendered graph post-render, decorating `.flowchart-link` elements with per-mode color (direct=green, tee=blue, gated=amber, tripped=red), pulse animation on traffic, and a selected-edge message-log panel.

**What shipped:** `broker.py` (+routing engine, circuit breaker, `send_scoped`, `EdgeStat`), `sdk_teammate.py` (in-process `send_to` MCP server), `factories.py` (`neighbors=` threading), `teammate_prompt.py` (neighbor section), `ui_server.py` (+3 routes with multi-instance proxy), `dashboard.html` (`TopologyEdgePanel`). 4 tasks, 13 ATs, +75 feature tests (edge-routing 19, circuit-breaker 17, scoped-send 22, edge-dashboard 17), 1537 full-suite green (exit 0).

- Pipeline impact: wsc-m2 → done. Next: wsc-m1.

### 2026-06-12 — Workflow Shape Composition M1.5 — Shipped

M1.5 fixes the shape-gate's interaction model: `propose_shape` is **non-blocking by default** (returns `{ok, shape_id, status:"pending"}` immediately; the lead stays free to supervise and chat while approval is pending). A new `resolve_shape(shape_id, decision)` lead tool gives operators a **chat-channel approval path** — approve over text or phone without opening the dashboard. `list_pending_shapes` lets the coordinator describe any pending gate over chat before the operator decides. Tool count 14 → 16.

**Single choke-point notify:** `broker.resolve_proposal` now drops a `{type:"shape_resolved", shape_id, status}` envelope into the lead's inbox on every resolution — whether it came from `resolve_shape` (chat) or `POST /shape-approval` (dashboard). The lead's `get_messages` loop wakes; no polling or blocking required.

**Resurfaceable gate:** the M0 `ShapeGatePanel` (visual unchanged) is now a persistent surface in `MCTopBar`. A pending gate shows a badge counting pending proposals (cross-instance `flatMap`); clicking opens or re-opens the modal; the badge persists across instance-switch and on close; it clears only on resolution. Fixes the M0 bug where the gate vanished on instance-switch.

**Human-in-the-loop softening (intentional):** putting `resolve_shape` on the lead surface means the coordinator *can* resolve a gate it proposed. The M0 mechanical barrier (UI-only) is replaced by trust-enforcement — the bridge to M1's blessed/trusted-shape auto-approval. The one un-softened invariant: `instantiate_shape` still refuses every non-`approved` proposal.

**What shipped:** `server.py` (+2 tools, non-blocking `propose_shape`), `broker.py` (lead-notify in `resolve_proposal`), `ui_server.py` (`name`/`summary` on `shape_proposals`), `dashboard.html` (MCTopBar badge + resurfaceable gate). 4 tasks, 6 ATs, 1461 full-suite green (exit 0), 5 Playwright cross-instance tests. Live UX verified by Jerome 2026-06-12: 0.52ms non-blocking register; cross-instance badge on leader; DAG in modal; APPROVED.

- Vision shift: none — advances the gate interaction model as planned.
- Pipeline impact: wsc-m1.5 → done. Next: wsc-m2.
...
Operator hit an empty config panel on the dashboard for a teammate spawned from a project-scope plugin install (`scope: "local"`, `projectPath: <repo>`). The teammate ran fine; only the transparency surface was broken. Spike confirmed root cause was unrelated to either of the two code-read hypotheses (synthetic-AgentDef divergence, namespace mismatch): `_read_installed_plugins` was filtering local-scope plugins by **strict equality** between `projectPath` and the resolved cwd. When claude-crew was started in a *subdirectory* of `projectPath`, the plugin was silently dropped — no agents in `merged_pack`, empty config panel, no operator-visible signal. Claude Code itself treats project-scope as a prefix (the plugin owns the whole tree); claude-crew was stricter than the host.

Fix shipped via RepoReactor as a single-task slice: replaced strict equality with `is_relative_to` against the normalized paths in `_user_loader._read_installed_plugins`, and added a new `plugin_scope_miss` startup-diagnostic category (slot in the classifier table ahead of the existing `installPath` rule so the H1 escape WARN keeps its `plugin` bucket). 8 acceptance tests pin exact match, subdirectory match, sibling rejection, parent rejection (with `scope_miss` WARN), symlink resolution, user-scope unaffected, classifier ordering. Full suite 978/0/23.

H1 (synthetic-AgentDef vs `_resolve_agent_def` divergence in `factories.py`) was a code-read finding from the same spike; not the operator's incident, real bug regardless. Split to BACKLOG as a follow-up.

### 2026-05-01 — #22 Current-Tool Badge Prominence — Shipped

Merged to master 2026-05-01. Closes the in-flight half of the "operator stares at silence during a 90s Bash" problem statement that #19 deliberately left open (#19 SC-7 scoped the tool-event stream to completed-only entries; #19 D-13 deferred the in-flight badge to a follow-up; that follow-up is this feature).

**Two-layer operator-silence signal** — both layers added in the same feature, neither sufficient alone:
- **Column-level scan affordance (SC-1 / D-1)** — 3px accent bar at the very top of each `AgentStreamColumn`, `--st-tool`-tinted at opacity 0.5 when in-flight, full opacity + `pulse` 1.6s animation when displayed-elapsed ≥5s. Operator scans column-tops left-to-right and finds the active-tool column without reading text.
- **In-column investigation chip (SC-2/3/4 / D-2/D-6/D-7)** — dedicated 22px `flexShrink: 0` row between status row and message scroll, always reserved height (D-10 prevents Pre/Post layout strobe). Renders `tool_name · {N}s [+M]` for in-flight, `tool_name · {duration}s ✓` for the 500ms settle frame after completion (D-7 with `useEffect` cleanup to prevent timer leaks on column unmount).

**What shipped (#22):**
- Four additive `/api/state` payload fields, no protocol break. Per-instance `now_wallclock` (D-4 — single `time.time()` per `_build_local_instance` call). Per-agent `oldest_in_flight` (D-3 — explicit-key allowlist, `args_summary` MUST NOT ship even on future redactor regression — sentinel MF-1 catch). Per-agent `in_flight_count` (convenience scalar). Per-agent `last_tool_completed` (settle-frame data path).
- D-9 tombstone-race invariant: `_build_local_instance` reads `info.alive` and `snap["current_tools"]` from the SAME `BrokerSnapshot` (D-11/SC-10 from #18); broker enforces `_close_open_tools` runs in `_tombstone_teammate` step 8b before tombstone visibility. No ghost badges possible by construction. Verified by `TestF22TombstoneRace` against a real broker `kill_teammate` path AND by E2E `test_killed_mid_tool_no_ghost_badge`.
- D-5 monotonic clock contract: `useTick1s` is `setInterval(1000)` (NOT `requestAnimationFrame` — keeps counting in backgrounded tabs); `computeElapsedSeconds` uses `performance.now()` for inter-poll smoothing (NOT `Date.now()` — immune to NTP step / laptop suspend / tab visibility). Server-stamped `now_wallclock` is the truth anchor; client snaps to it on each WebSocket message and ticks locally. Defended by negative-assertion tests scoped to the `computeElapsedSeconds` function body.
- D-8 API surface freeze: `tools[]` is the full-set name list (frozen, no new consumers); `current_tool` retains last-started semantic for SC-9 back-compat; `current_tools[]` is the canonical structured list; `oldest_in_flight` is the badge field. One-line code comment on the agent dict; freeze recorded here as well so future features designing against tool data know which field to consume.
- 25 net new tests across 4 classes (`TestF22BadgePayload`×11, `TestF22TombstoneRace`×1, `TestF22DashboardHtml`×7, `TestE2EBadgePipeline`×6). Two pre-existing regression-guard tests extended to include the new fields (`test_dashboard_payload_shape_unchanged_post_refactor`, `test_agent_has_required_fields`). 610 passing, 11 skipped (live SDK).

**Process highlights:**
- **Phase 1 three-pushback warmup is now a TEMPLATE pattern, not a one-off.** Second feature in a row (#19 was the first) where co-architect named all three Phase 2 design pillars BEFORE Phase 1 SCs were drafted. Pillar 1 → SC-5 oldest-in-flight semantics. Pillar 2 → SC-6 + D-4/D-5 clock pairing. Pillar 3 → D-1 column-level affordance + D-2 chip-level chip. Phase 2 became synthesis, not discovery; Phase 4 became near-mechanical execution. Cross-feature lesson: the warmup belongs at Phase 1, before SCs exist.
- **Sentinel-as-pseudocode-reader caught MF-1 at Phase 2 cost.** D-3's prose said "without args_summary" but the carried-into pointer was descriptive, not concrete. Sentinel flagged it; D-3 was rewritten to spell out the explicit-key allowlist. A naive copy-then-delete implementation would have shipped the field on the wire on day one. Caught at spec time, zero implementation cost.
- **Sentinel mid-build checkpoint at T3-T4 boundary caught two regression-guard holes.** Pre-existing tests (`test_dashboard_payload_shape_unchanged_post_refactor.required_instance_keys`, `test_agent_has_required_fields`) had stale field lists that LOOKED like comprehensive guards but didn't include the new F22 fields. Updated in the same commit as T5. The "tests cover feature behavior, not just pre-existing" lens at sentinel review keeps surfacing this class of drift.
- **Vacuous-loop test caught at sentinel review.** Initial `test_oldest_in_flight_none_for_tombstoned_teammate` had a loop guard that was always-true given a preceding assertion. Tightened to an unconditional structural check. Adding "Are any assertions inside loops vacuously true under preceding assertions?" to the standard sentinel review prompt.
- **Negative-assertion scoping.** First version of `test_uses_performance_now_for_elapsed` only asserted positive presence of `performance.now()`. Sentinel flagged the negative claim wasn't tested. Tightened to scope the `assertNotIn("Date.now()", ...)` check to the `computeElapsedSeconds` function body — `fmtAgo` legitimately uses `Date.now()` for relative-time-ago. Pattern: when asserting "X is used," also assert "Y is not used in path Z," not file-globally.
- **Agent persistence trap.** Spawned co-architect-22 / sentinel-22 with `name:` thinking they'd be cross-turn addressable; they weren't (Agent calls return-and-end). Workaround: spawn a fresh agent each phase gate with explicit FEATURE.md re-load. Worked fine. Document in TEMPLATE under "Phase gates."

**Cost / time:** ~3 hours wall on top of the post-#19 retro context still warm. ~$2 estimated token spend (co-architect Opus + sentinel Sonnet + 3 Haiku Phase 1 explorers). 5 commits on the feature branch + merge commit (T1, T2, T3, T5+checkpoint, retro+journal+merge).

**Pipeline impact:**
- #22 → done. Capability #4 status flipped from "substantially met — message and tool observability complete; envelope kind enrichment (#16) and in-flight badge prominence (#22) deferred" to "substantially met — message + tool + in-flight observability complete; envelope kind enrichment (#16) deferred."
- Operator-silence problem statement (the cross-feature throughline since #8/F8) is now structurally closed: completed-tool stream (#19) + in-flight badge prominence (#22) cover both temporal halves.
- Co-architect MERGE-WITH-CONFIDENCE; sentinel MERGE-WITH-NOTE (M-1 fixed in branch by tightening the negative-assertion test; M-2 logged to BACKLOG as `test_e2e_multi_instance.py` follow-up: ~10-line addition asserting unreachable instance dicts have no `now_wallclock` on the wire).
- Next pipeline work: #24 (subagent-dispatch routing guard + post-output validation) addresses the fail-soft fabrication pathology surfaced 2026-05-01. #16 (envelope kind typing) and #25 (startup diagnostics on dashboard) are polish.

### 2026-05-01 — #19 Tool-Use Events in Dashboard Stream — Shipped

Merged to master 2026-05-01 (`db2d0ab`). The Mission Control dashboard now shows tool calls inline in the per-agent transcript stream as `kind: "tool"` entries alongside lead↔teammate envelopes, sorted by raw-float timestamp. Closes the data-pipeline half of the "what is this teammate actually doing right now" gap at the tool level. Capability #4 advances from "message + status observable" to "message + status + tool-event observable."

**What shipped:**
- `ToolEvent` frozen dataclass mirroring #8's JSONL `tool_end` field shape exactly (D-1) — one shape, two surfaces.
- Per-teammate bounded `collections.deque(maxlen=200)` on `Teammate` base class, env-overridable via `CLAUDE_CREW_TOOL_EVENTS_PER_TEAMMATE` (D-2).
- Three append sites: `_on_post_common` (success + failure), `_close_open_tools` (death/kill/turn_end). `orphan_post` filtered (D-3). In-memory append happens BEFORE transcript write (D-4) — disk-full does not blind the dashboard (SC-8).
- `_tombstone_teammate` step 8c uses `dataclasses.replace` to capture the deque into `TeammateInfo.tool_events_at_death` AFTER `_close_open_tools` (step 8b) runs — so abandoned/killed events at tombstone time land in the captured tuple (sentinel F1, D-7).
- `BrokerSnapshot.tool_events` tightened from `tuple[Any, ...]` to `tuple[ToolEvent, ...]`. `Broker.snapshot()` flattens live + dead per-teammate events with stable sort by `finished_at_wallclock` (D-5, D-6).
- UIServer merges into per-crew transcript stream as `{t, from, to: None, kind: "tool", body}` records, filtering Task at render time (Q2 revised — preserve in snapshot, hide in dashboard to avoid double-rendering #7). Sort on raw float, not on `_ts()`-truncated string (sentinel D1 catch).
- No `/api/state` schema change — events ride inside `transcripts[crew_id]` (D-10). Remote-fanout pass-through carries them automatically; SC-6 stale-on-failure inherits #13's existing behavior unchanged.

**Process highlights:**
- **Co-architect's three-pushback warmup (Phase 1) loaded all three Phase 2 design pillars before they were needed.** Flat-tuple-vs-retention-math → D-6 sort. SC-1 freshness budget → D-11 stage breakdown. SC-7 doesn't fully solve operator-silence → BACKLOG D-13 + escalation to a proper pipeline row at merge.
- **Sentinel review at T2-T3 boundary (not T3-T4) per co-architect's recommendation** caught five real gaps cheap: turn_end coverage, interrupted outcome coverage, parallel-tools coverage, missing assertion + cosmetic typing tighten. Reviewing T2 (highest-risk task — 3 hook callsites + tombstone state mutation) before T3 froze the dataclass shape, so T3-T4 cleanup was final-pass scope.
- **Pre-implementation sentinel found two spec bugs that would have broken at runtime:** F1 (Phase 2 pseudocode mutated a frozen dataclass — would have FrozenInstanceError'd) and F2 (StubTeammate / `_NoopTeammate` test fixtures needed the deque init or 550+ tests would have AttributeError'd). Both were spec-time fixes, zero implementation cost. Sentinel-as-pseudocode-reader pays off when the prompt asks "what would break at runtime if I implemented exactly this."
- **D-4 ordering as load-bearing decision.** "In-memory append BEFORE transcript write" is the entire mechanism behind SC-8 (disk-full doesn't blind dashboard). Encoded as 2-line code ordering with named tests at both unit and E2E levels. Impossible to drift silently.
- **No `/api/state` schema change is a feature, not a missing-feature.** Tool events ride inline; #13's fanout pass-through carries them for free. Simplest extension wins.
- **Live-spawn check at end:** general-purpose teammate fired Read tool, `last_tool_completed` populated, deque path verified by code-adjacency. Not a full dashboard test (Jerome will do that), but adequate to confirm the producer side.

**Cost / time:** ~3 hours wall, $1-2 estimated token spend (co-architect Opus + sentinel Sonnet + 2 Haiku Phase 2 explorers). 6 commits on the feature branch + merge commit. 35 net new tests (550 → 585).

**Pipeline impact:**
- #19 → done.
- #22 (current_tool badge prominence) added as the next dashboard-UX feature row, NOT buried in BACKLOG. Co-architect's MERGE-WITH-NOTE flag: this is the deferred half of #19's "operator silence during 90s Bash" problem statement, schedule it before other polish work.
- Capability #4 status: "substantially met — message and tool observability complete; envelope kind enrichment (#16) and in-flight badge prominence (#22) deferred."

### 2026-04-30 — #21 Teammate Prompt Parity + F14 Token-Telemetry Fix — Shipped

Merged to master 2026-04-30 (`1e3cfdc`). Closes the gap surfaced during #18: teammates spawned via `spawn_teammate` were getting an 8-word default system prompt that discarded the pack body entirely — no role identity, no awareness of available subagents, no delegation framing. Result: teammates that *had* Task tool access (the SDK adds it implicitly when `agents=` is passed) didn't know to use it.

**What shipped (#21):**
- New `claude_crew/teammate_prompt.py` — `build_teammate_prompt(role, pack_body, agents)` assembles `pack_body + addendum` with four sentinel-bounded sections: context override (corrects leaf-context language), dynamic peer list (sorted, self excluded), delegation framework (with cite-vs-research clause), anti-patterns.
- Leaf assertion factored out — `_LEAF_SUFFIX` lives in the subagent loader and is appended at Task-invocation time. Pack body becomes shared role identity, true in both contexts.
- `parse_pack_text` returns 4-tuple `(key, agent, fm, body)` — raw body now exposed for teammate-context assembly.
- 15 new prompt-parity tests (`test_teammate_prompt.py`) covering negative-pattern lint, sentinel ordering, peer-list R-1/R-2/R-3, explorer hint EC-11/R-7, leaf-suffix on subagent path, SDK integration.
- Live behavioral probe (`test_live_teammate_delegation.py`) — gated by `CLAUDE_CREW_LIVE_TESTS=1`. Asserts `last_subagent_completed` populates after a real teammate processes a research-shaped task. Pre-flight check fails fast if any sentinel is missing from the assembled prompt (no wasted live API spend on regressions).

**Live verification:** spawned a clean general-purpose teammate, sent "give me a 2-sentence assessment of how heavily claude-crew relies on asyncio — don't quote files." Status snapshot showed `last_subagent_completed.tool_use_id` set after the turn — the teammate dispatched the Agent tool (subagent), got the result, synthesized. Cite-vs-research clause working as designed: an earlier file-find probe in the same lifecycle correctly read directly (the answer WAS the file:line list).

**F14 bug discovered mid-test, fixed in same branch (commit `0cd9c21`):**
- Live probe surfaced impossible-looking telemetry: turn 1 input=28,541 / cost=$0.0636, turn 2 input=14,580 / cost=$0.0732. Tokens dropped while cost rose monotonically.
- Root cause: F14 assumed `ResultMessage.usage` was session-cumulative and overwrote the snapshot each turn. Reality: `usage` is per-turn (one API call), `total_cost_usd` IS cumulative (SDK aggregates including subagent billing).
- Fix: tokens accumulate (`+=`), cost overwrites (`=`). Renamed `TurnDrainResult.cumulative_input/output_tokens` → `turn_input/output_tokens` for accurate semantics.
- Bonus clarity: parent `usage` only counts the parent's context window — subagent internal tokens are separate. So `_total_input_tokens` is now exactly "context used by top-level teammate," which is what operators actually want. Cost remains "all-in session spend including subagents." Two distinct, accurate signals.
- Regression test `test_token_totals_accumulate_across_turns_even_when_per_turn_decreases` drives 100→200→50 input and asserts the snapshot reaches 350 (would have stopped at 50 under old semantics).

**Process highlights:**
- **Light SDD as predicted by co-architect.** No Phase 3 task list ceremony — Phase 1 brief + Phase 2 mini-spec + 2 implementation tasks (T1 + T2). T3 (cite-vs-research clause) was added after manual testing exposed a false-positive: the teammate was delegating reads it should have done directly. Refinement landed mid-build, no re-spec needed.
- **Live-test pre-flight check pattern.** The `assert SENTINEL_* in _system_prompt` block in `test_live_teammate_delegation.py` runs before the live API call. A future regression in prompt assembly fails the test in <1s without paying for tokens. Worth lifting as a pattern for any feature with both static + live tests.
- **Bug discovery via the substrate observing itself.** F14 was shipped clean (its own tests passed) but only the in-the-wild `general-purpose` teammate exercising real multi-turn cost monotonicity revealed the bug. The substrate's own dashboard surfaced it. Pattern: feature dogfooding catches what unit tests architecturally cannot.
- **No breadcrumbs.** Both bugs (delegation prompt, token semantics) were closed in the same branch, not deferred to BACKLOG. Jerome's directive: if we have context and access, fix it now.

**Pipeline impact:** #21 → done. F14 corrected. With teammate delegation now functional, the next real-task validation run (MMM-4b) gets a substrate where teammates actually use their subagents — should significantly reduce parent context bloat for research-heavy roles.

### 2026-04-30 — #18 Broker Snapshot API + Dashboard Polish — Shipped

Merged to master 2026-04-30 (`2a93977`). Closes the long-standing fragility where `UIServer` reached into `Broker._info`, `_teammates`, `_log`, plus the public `crew_id` and the hidden `teammate._model` private read. `Broker.snapshot()` is the public read API; UIServer is broker-decoupled at function-input level (`_build_local_instance(snapshot)`). Bundled with the long-deferred dashboard polish: real git branch detection with 30s TTL cache replaces hardcoded "main" in the local instance row; the dead `_get_redaction_version()` ImportError fallback is gone (T1 of F8 shipped weeks ago).

**Snapshot design highlights:**
- `BrokerSnapshot` is a frozen dataclass; status dicts are deep-copied at build (D-2) so callers cannot reach back into live teammate state via inner refs (`last_tool_completed`, `last_subagent_completed` were the gotcha)
- Sync execution + single-threaded asyncio = atomicity guarantee without locks (D-1)
- `tool_events: tuple[Any, ...] = ()` field reserved for #19, with documented flat-tuple contract — additive, no v2 needed when #19 lands

**Process learning surfaced:** the persistent Opus co-architect was burning 1.3M input tokens per design pass by reading raw files in its own context — the system prompt never told it to delegate to explorer subagents. Killed and respawned with one-line operating-style guidance in the spawn prompt; per-review token use dropped 83%. This exposed a structural gap (Feature #21, queued as next) where teammate system prompts are a generic 8-word default that throws away the pack-file body entirely. The "operate, observe gaps, log them" loop continues to find architectural friction nobody planned to look for.

**Pipeline impact:** #19 is now unblocked structurally — its broker-level read surface fits cleanly into the snapshot's reserved `tool_events` field. #21 promoted to next based on this session's discovery.
