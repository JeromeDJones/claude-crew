...
**Next up**: #15 native Claude Code agent format compatibility *(SDD entering)* · #24 subagent-dispatch routing guard + post-output validation · #16 message kind typing · #25 startup diagnostics on dashboard *(idea, surfaced from #23 OQ-1)* · #20 peer messaging between teammates *(idea)*
...
| 15 | **Native Claude Code agent format compatibility.** Original framing ("expanded default subagent pack — add `reviewer` and `runner`") was wrong-abstraction: operators distributing claude-crew already have rich `~/.claude/agents/*.md` definitions; substrate's job is to consume them, not bundle alternatives. Tier 1 work: `name:` silent-accept + canonical role key (file stem fallback); `model:` optional (SDK default when absent); `tools:` optional + comma-string format support; `color:` silent-accept; system prompt composition revised — claude-crew substrate guidance LEADS, agent body APPENDED (today's `_LEAF_SUFFIX`-as-trailing-suffix is wrong, both in position and framing). Closes the noisy-startup-WARN regression #17 BACKLOG M-2 flagged. Live dogfood: load `~/.claude/agents/runner.md` with zero WARNs and zero load failures. Deferred to #24 / hook-aware feature: `hooks:` per-agent lifecycle hooks; `isolation: worktree`. | 1, 2 | 4 | S-M | **next** | Reframed 2026-05-01 after authoritative claude-code-guide WebFetch confirmed Claude Code's full agent frontmatter surface (16 fields). Spec at `doc/features/FEATURE-claude-code-agent-format-compatibility.md`. Vision lines 205-216 ("Hooks: two systems, two answers") incorrectly states `AgentDefinition` has no `hooks` field — Claude Code DOES support it; correction will land with this feature. |
...
- Next dashboard work: #15 (reviewer + runner role packs) is the highest-value of the remaining `next` items because real-task builds keep instantiating those roles without pack definitions. #16 (envelope kind typing) and #17 (PackFrontmatter parity) are polish.
