...
Makes crew shapes first-class, declarative, and human-gated. M0 shipped 2026-06-11; M1.5 shipped 2026-06-12. Agreed execution order: **M0 done → M1.5 done → M2 next → M1 later** (Jerome). Rationale: M1.5 fixed the gate's *interaction model* (non-blocking, dual-channel, resurfaceable) — daily-use friction hit live; it was self-contained. M2 then makes the approved graph *execute*; M1 (shape templates, classifier) is independent and higher-value after the graph runs.
...
| wsc-m2 | **Workflow Shape Composition M2.** Edge routing enforcement: `tee`/`direct`/`gated` modes actually constrain message flow. Scoped teammate `send_to` — messages only traverse declared edges. Neighbor adjacency injection — each spawned teammate's context includes its declared neighbors. Circuit breaker. Makes the approved graph execute; M0's `Topology` becomes the live routing rail. Edge animation/promotion in the dashboard also M2. | 1, 4 | 1 | L | **next** | Agreed after M0/M1.5 (Jerome). Plugs directly into `BrokerSnapshot.topologies` + `Topology.edges` rail M0 laid. |
