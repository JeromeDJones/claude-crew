...
**Features Implemented**: 15 + post-#13 polish + per-agent dashboard tokens (MVP + #6 telemetry-based liveness + #7 subagent-activity envelopes + #8 tool-execution telemetry + #9 get_messages long-poll + #10 agent-config-extension + #11 lightweight-subagent-context + #12 mission-control-ui + #13 multi-instance-registry + leader election + race-free port binding + dashboard UX polish + #14 token/cost telemetry + #18 broker snapshot + dashboard polish)
**Next up**: #23 global skills for SDK teammates *(Phase 2 locked, entering Phase 3)* · #17 agent definition parity · #24 subagent-dispatch routing guard + post-output validation · #15 expanded subagent pack · #16 message kind typing · #25 startup diagnostics on dashboard *(idea, surfaced from #23 OQ-1)* · #20 peer messaging between teammates *(idea)*
...
| 23 | **Global skills support for SDK teammates.** User-defined skills in `~/.claude/skills/*/SKILL.md` and project `.claude/skills/*/SKILL.md` are invocable by SDK teammates the same way the lead Claude Code session invokes them. Today teammates load user/project agents (#10/#11) but the skill surface is invisible — operators have invested in skills like `/sdd-workflow`, `/security-review`, `/docs-maintain`, and that work doesn't transfer to spawned teammates. As role packs expand (#15 reviewer/runner) and real-task crews grow, having teammates run skills naturally extends the operator's existing toolkit into the crew. Phase 1 must spike: how does Claude Agent SDK expose skills (config option, MCP, tool surface, prompt injection)? Discovery determines mechanism. | 1, 2 | 4 | M | **next** | Surfaced 2026-05-01 during /crew-showcase work — natural follow-on after #11 (lightweight subagent context) and #21 (teammate prompt parity) shipped the agent surface; skill surface is the remaining parity gap with the lead session. |
