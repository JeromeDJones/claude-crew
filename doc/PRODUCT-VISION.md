...
**Features Implemented**: 16 + post-#13 polish + per-agent dashboard tokens (MVP + #6 telemetry-based liveness + #7 subagent-activity envelopes + #8 tool-execution telemetry + #9 get_messages long-poll + #10 agent-config-extension + #11 lightweight-subagent-context + #12 mission-control-ui + #13 multi-instance-registry + leader election + race-free port binding + dashboard UX polish + #14 token/cost telemetry + #18 broker snapshot + dashboard polish + #17 agent definition parity)
**Next up**: #16 partial (thinking half — cut or ship?) · dead-teammate UI segregation *(specced, Phase 3 gate)* · #25 startup diagnostics on dashboard *(idea, surfaced from #23 OQ-1)* · #20 peer messaging between teammates *(idea)*
...
| 16 | **Message kind typing in transcript stream.** Transcript envelopes currently all emit `kind: "msg"`. Inspect payload shape to emit `kind: "tool"` for tool-call envelopes and `kind: "thinking"` for thinking-block envelopes. Dashboard stream columns then show the visual distinction (monospace pill vs. italic) the design intends. Requires coordination between broker envelope format and `UIServer._build_local_instance()`. | 4 | 3 | S | **partial** | `kind: "tool"` **done** — shipped as part of #19 (`ui_server.py:308`). `kind: "thinking"` **not done** — `ThinkingBlock` responses are silently dropped by `sdk_teammate.py` (A2, line 159); nothing in the transcript stream emits thinking entries today. Remaining scope: decide whether thinking blocks are worth surfacing (extended-thinking mode only; standard mode rarely produces them) or cut the thinking half and close #16 as done. |
