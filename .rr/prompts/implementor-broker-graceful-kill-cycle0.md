...
Implement task `broker-graceful-kill` (index 2) of the breakout against
...
Task index: `2`
Description: Add `graceful`/`flush_timeout` params to broker.kill_teammate; insert the pre-tombstone flush step (skip when not graceful, teammate not alive, or no memory surface). Add the `_terminating` id set and bounce new sends arriving during the flush window. Leave _handle_teammate_death routing straight to _tombstone_teammate (no flush). Preserve D2 ordering, double-kill idempotency, and timeout fall-through to hard tombstone.
...
Build report (write here): `/home/jerome/dev/claude-crew/.rr-worktrees/graceful-termination-memory-flush/.rr/reports/graceful-termination-memory-flush-task-broker-graceful-kill-build-0.md`
...
Run `cd "/home/jerome/dev/claude-crew/.rr-worktrees/graceful-termination-memory-flush/.rr-worktrees/broker-graceful-kill"` before any file operation. Treat this path as binding.
...
6. Write the build report to `/home/jerome/dev/claude-crew/.rr-worktrees/graceful-termination-memory-flush/.rr/reports/graceful-termination-memory-flush-task-broker-graceful-kill-build-0.md` using the build-report
...
7. Emit this as the **final line** of your response — no trailing text after it:
   `RR-VERDICT: PASS|FAIL|BLOCKED <slug> <cycle> /home/jerome/dev/claude-crew/.rr-worktrees/graceful-termination-memory-flush/.rr/reports/graceful-termination-memory-flush-task-broker-graceful-kill-build-0.md`
