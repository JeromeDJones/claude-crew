...
def text_response_with_openai_usage(
    text: str,
    *,
    prompt_tokens: int,
    ...
    """text_response variant that scripts ResultMessage with OpenAI-shape usage.

    Used for local backend (ccr → llama.cpp) tests. The ResultMessage.usage
    dict uses OpenAI key names: prompt_tokens / completion_tokens /
    prompt_tokens_details.cached_tokens.

    prompt_tokens: full per-turn input total (already includes cached tokens;
        do NOT add cached_tokens again — that would double-count).
    completion_tokens: per-turn output token count.
    cumulative_cost_usd: session-cumulative cost (0.0 for local backends).
    cached_tokens: optional; stored in prompt_tokens_details.cached_tokens.
    invocation_inputs: optional list of per-invocation input token counts.
        When provided, emits one AssistantMessage per entry using OpenAI-shape
        usage (prompt_tokens key), exercising the peak-invocation path.
        When None, a single AssistantMessage with no usage is emitted.
    """
    usage: dict[str, Any] = {
    ...
    if invocation_inputs is None:
        # Legacy single-AssistantMessage path — no per-invocation usage.
        assistant_msgs: list[Any] = [
            AssistantMessage(content=[TextBlock(text=text)], model="fake-local-model"),
        ]
    else:
        # Multi-invocation path — one AssistantMessage per invocation with
        # OpenAI-shape usage. Only the LAST carries the text payload.
        assistant_msgs = []
        for i, inv_input in enumerate(invocation_inputs):
            is_last = i == len(invocation_inputs) - 1
            content_blocks = [TextBlock(text=text)] if is_last else []
            assistant_msgs.append(
                AssistantMessage(
                    ...
                    usage={"prompt_tokens": inv_input, "completion_tokens": 0},
                )
            )
